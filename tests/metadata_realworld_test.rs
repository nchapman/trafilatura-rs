...
fn metadata_mock_files() -> HashMap<&'static str, &'static str> {
    ...
    m.insert(
        "https://www.mercurynews.com/2023/01/16/letters-1119/",
...
fn load_metadata_html(url: &str) -> String {
    ...
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join(filename);
    let bytes = std::fs::read(&path).unwrap_or_else(|e| panic!("Failed to read {filename}: {e}"));
...
fn test_metadata_real_pages() {
    ...
    assert!(m
        .description
        .starts_with("En réaction à la candidature du conseiller recherche"));
    ...
    assert!(m
        .description
        .starts_with("Report claims higher education institutions"));
    ...
    assert!(m
        .title
        .contains("Berlin confronts Germany's colonial past with new initiative"));
    ...
    assert!(
        m.description
            .starts_with("Today, on Wikipedia\u{2019}s 19th birthday"),
        "got: {:?}",
        m.description
    );
    ...
    assert!(m
        .title
        .ends_with("scores historic upset at SAG awards, boosting Oscar chances"));
    assert_eq!(m.author, "Jill Serjeant");
    assert_eq!(
        m.date.map(|d| d.format("%Y-%m-%d").to_string()).as_deref(),
        Some("2020-01-20")
    );
    ...
    assert_eq!(
        m.url,
        "https://www.reuters.com/article/us-awards-sag-idUSKBN1ZI0EH"
    );
    ...
    assert!(m
        .description
        .starts_with("The rocky island of Redonda, once stripped of its flora and fauna"));
    ...
    assert_eq!(
        m.title,
        "Carrie Lam should study Tsai Ing-wen\u{2019}s playbook"
    );
    ...
    assert_eq!(
        m.title,
        "Wie kann ich schnell abnehmen? Der Schlachtplan zum Wunschgewicht"
    );
    ...
    assert!(
        m.description
            .starts_with("We\u{2019}re spending Women\u{2019}s History"),
        "got: {:?}",
        m.description
    );
